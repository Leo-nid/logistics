...
def dist(node_lhs: Node, node_rhs: Node, is_haversine = True):
  if is_haversine:
    return haversine.haversine((node_lhs.latitude, node_lhs.longitude), (node_rhs.latitude, node_rhs.longitude), unit=haversine.Unit.METERS)
...
def dist_to_segment(stop_lhs: Node, nodes: list[Node], is_haversine = True):
  ...
  if dp >= 0 and dot_product(nodes[0] - nodes[1], stop_lhs - nodes[1]):
    ...
    return part, dist(stop_lhs, closest_point, is_haversine), sgn
  else:
    dsts = [dist(stop_lhs, nodes[i], is_haversine) for i in range(2)]
